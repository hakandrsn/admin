...
const User = (props) => {
    ...
    return (
        <div className='flex-shrink-1 flex-grow-1 rounded py-2 px-3 d-flex flex-column gap-1' style={{ flexBasis: 200, backgroundColor: "#ffeaa7" ,fontSize:12}}>
            <Link to={`user/${data._id}`} className='d-flex align-items-center text-decoration-none' style={{color:"black"}}>
                <BsFillPeopleFill size={18} style={{ marginRight: 4 }} />
                <label className='text-capitalize fs-4'>{data.firstname + " " + data.lastname}</label>
            </Link>
            <div className='d-flex align-items-center'>
                <FaTransgender size={18} style={{ marginRight: 4 }} />
                <label>{data.gender == "male" ? "Erkek" : "Kadın"}</label>
            </div>
            <div className='d-flex align-items-center'>
                <BsFillTelephoneFill size={18} style={{ marginRight: 4 }} />
                <label>{data.phone || "Yok"}</label>
            </div>
            <div className='d-flex align-items-center'>
                <GoKey size={18} style={{ marginRight: 4 }} />
                <label>{data.tc || "Tc Yok"}</label>
            </div>
            <div className='d-flex align-items-center'>
                <BiCurrentLocation size={18} style={{ marginRight: 4 }} />
                <label>{data.location || "Konum bilinmiyor"}</label>
            </div>
            <div className='d-flex align-items-center'>
                <BsMailbox2 size={18} style={{ marginRight: 4 }} />
                <label>{data.role || "Bilinmiyor"}</label>
            </div>
           
            <div className='d-flex justify-content-between'>
            <div className='d-flex align-items-center'>
                <ImSortNumbericDesc size={18} style={{ marginRight: 4 }} />
